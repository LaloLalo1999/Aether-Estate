...
export function TransactionActions({ transaction }: TransactionActionsProps) {
  ...
  const { mutate: deleteTransaction, isPending: isDeleting } = useMutation<{ id: string; deleted: boolean }, Error, string>({
    mutationFn: (id) => api(`/api/transactions/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
  ...
  return (
    <>
      ...
      <EditTransactionSheet transaction={transaction} isOpen={isSheetOpen} onOpenChange={setIsSheetOpen} />
